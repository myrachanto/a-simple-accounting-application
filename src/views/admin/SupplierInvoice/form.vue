  ...
  <back>
    <v-form>
      <v-container class="fill-height">
        ...
        <v-row>
          <v-col cols="12" md="4">
            <v-textarea clearable clear-icon="mdi-close-circle" label="Instructions" v-model="sinvoicedetails.instructions" required></v-textarea>

          </v-col>
          <v-col cols="12" md="4">
            <v-textarea clearable clear-icon="mdi-close-circle" label="Terms and conditions" v-model="sinvoicedetails.terms" required></v-textarea>
          </v-col>
        ...
        <v-row align="center" justify="center">
          <v-col cols="12" md="12" align="center" justify="center">
            <v-btn color="red" @click="cancelsinvoice(code)">Cancel the sinvoice</v-btn>
            <v-btn color="green" @click="save(code)">Process the sinvoice</v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-form>
<v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title>
          <span class="headline">Add expence to the sinvoice</span>
        </v-card-title>
        <v-card-text>
         <v-container>
            <v-row>
...
export default {
  ...
  data() {
    return {
      ...
      taxlist: [],
      title: 'Create',
      init: 'api/sinvoice/view',
      addcart: 'api/scarts',
      cartview: 'api/scarts/view',
      redirect: '/sinvoice',
      store: 'api/sinvoice',
      cancel: 'api/scarts/cancel',
      method: 'post',
      snackbar: false,
      timeout: 3000,
      product: {},
  ...
  methods: {
    ...
    async addproduct() {
      try {
        ...
        let fd = new FormData();
        fd.append("suppliername", this.supplier.name)
        fd.append("suppliercode", this.supplier.suppliercode)
    ...
    getsupplier(name) {
      console.log(name)
      const supplierinfo = this.suppliers.find(supplier => {
    ...
    async save(code) {
      try {
        ...
        let fd = new FormData();
        fd.append("suppliername", this.supplier.name)
        fd.append("terms", this.sinvoicedetails.terms)
        fd.append("code", code)
        fd.append("suppliercode", this.supplier.suppliercode)
        fd.append("instructions", this.sinvoicedetails.instructions)
