        ...
        <v-row>
         ...
         <v-row align="center" justify="center">
          <v-col cols="12" md="12" align="center" justify="center">
            <v-btn color="red" @click="cancelcredit(invoice.code)">Cancel the credit note</v-btn>
            <v-btn color="green" @click="savecredit(invoice.code)">Process the credit note</v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-form>
  ...
  methods: {
    ...
    async saveExpence() {
      try {
        ...
        fd.append("code", this.invoice.code);
    ...
    async savecredit(code) {
      try {
        let fd = new FormData();
        fd.append("code", code);
        await axios.post("api/invoice/credit", fd, {
          "Content-Type": "multipart/form-data",
        });
    ...
    async fetchData() {
      try {
        ...
        const { customer, invoices, transactions, expences,credits } = data;
